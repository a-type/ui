...
export function getButtonClassName({
	...
	return clsx(
		...
		emphasis === 'contrast' &&
			'layer-variants:(border-bg hover:bg-gray-ink focus-visible:bg-gray-dark)',
		...
		{
			'layer-variants:(px-4 py-1 text-sm rounded-md)': size === 'small',
			'layer-variants:[&[data-has-icon=true][data-has-label=false]]:(p-2.35 text-sm rounded-lg)':
				size === 'default' && !disableIconMode,
			'layer-variants:[&[data-has-icon=true][data-has-label=false]]:(p-2 text-xs rounded-lg -m-y-0.5)':
		...
		'layer-components:focus:outline-off',
		'layer-components:focus-visible:(outline-off bg-main ring-accent bg-darken-2 ring-6)',
		'layer-components:[&[data-focus=true]]:(ring-6 bg-darken-1)',
